...
export class ShopService {
  ...
  async injectSnippet(session: shopifySession, themeName: string) {
    try {
      ...
      const hasSnippet = await shopify.api.rest.Asset.all({
        session,
        theme_id: theme.id,
        asset: {"key": "snippets/reserve-timer.liquid"},
      });

      if (!hasSnippet.length) {
      ...
      if (theme && freeThemeIds.includes(theme.theme_store_id)) {
        ...
        if (!cartDrawerAsset[0].value.includes('{%  render "reserve-timer",')) {
        ...
        if (!mainCartItemsAsset[0].value.includes('{%  render "reserve-timer",')) {
